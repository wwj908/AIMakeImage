...
## Docker 后端（推荐，避免服务器 JDK 版本不匹配）

如果服务器没有 JDK 21，可直接使用容器运行后端：

```bash
docker rm -f aimakeimage-api 2>/dev/null || true
docker run -d \
  --name aimakeimage-api \
  --restart unless-stopped \
  --env-file /etc/aimakeimage/aimakeimage.env \
  -p 127.0.0.1:18080:18080 \
  -v /www/aimakeimage/backend:/app \
  -v /www/aimakeimage/storage:/www/aimakeimage/storage \
  -w /app \
  eclipse-temurin:21-jre \
  java -jar /app/make-image-api.jar
```
